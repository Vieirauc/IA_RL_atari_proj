...
class Agent_DQN(Agent):
    def __init__(self, env, args):
        ...
        self.build_net()
        t_params = tf.compat.v1.get_collection('tar_net_params')
        self.e_params = tf.compat.v1.get_collection('eval_net_params')
        self.update_tar_net = [tf.compat.v1.assign(t, e) for t, e in zip(t_params, self.e_params)]

        optimizer = tf.compat.v1.train.RMSPropOptimizer(self.args.lr, momentum=0, epsilon=1e-6, decay=0.99)
        # gradient clipping
        ...
        self.saver = tf.compat.v1.train.Saver(self.e_params, max_to_keep=20)
    ...
    def build_net(self):
        ...
        def build_layers(s, name):
            w_init = tf.compat.v1.truncated_normal_initializer(0, 1e-2)
            b_init = tf.compat.v1.constant_initializer(1e-2)
            with tf.compat.v1.variable_scope('conv1'):
                w_conv1 = tf.compat.v1.get_variable('w_conv1',[8,8,4,32],initializer=w_init,collections=name)
                b_conv1 = tf.compat.v1.get_variable('b_conv1',[32],initializer=b_init,collections=name)
                conv1 = conv2d(s, w_conv1, 4)
                h_conv1= tf.nn.relu(tf.nn.bias_add(conv1, b_conv1))

            with tf.compat.v1.variable_scope('conv2'):
                w_conv2 = tf.compat.v1.get_variable('w_conv2',[4,4,32,64],initializer=w_init,collections=name)
                b_conv2 = tf.compat.v1.get_variable('b_conv2',[64],initializer=b_init,collections=name)
                conv2 = conv2d(h_conv1, w_conv2, 2)
                h_conv2= tf.nn.relu(tf.nn.bias_add(conv2, b_conv2))

            with tf.compat.v1.variable_scope('conv3'):
                w_conv3 = tf.compat.v1.get_variable('w_conv3',[3,3,64,64],initializer=w_init,collections=name)
                b_conv3 = tf.compat.v1.get_variable('b_conv3',[64],initializer=b_init,collections=name)
                conv3 = conv2d(h_conv2, w_conv3, 1)
                h_conv3= tf.nn.relu(tf.nn.bias_add(conv3, b_conv3))
                h_conv3_flat = tf.reshape(h_conv3, [-1,3136])

            with tf.compat.v1.variable_scope('fc1'):
                w1 = tf.compat.v1.get_variable('w1', [3136,512], initializer=w_init, collections=name)
                b1 = tf.compat.v1.get_variable('b1', [512], initializer=b_init, collections=name)
                fc1 = tf.nn.relu(tf.nn.bias_add(tf.matmul(h_conv3_flat, w1), b1))

            if self.args.duel:
                with tf.compat.v1.variable_scope('Value'):
                    w2 = tf.compat.v1.get_variable('w2', [512,1], initializer=w_init, collections=name)
                    b2 = tf.compat.v1.get_variable('b2', [1], initializer=b_init, collections=name)
                    V = tf.nn.bias_add(tf.matmul(fc1, w2), b2)

                with tf.compat.v1.variable_scope('Advantage'):
                    w2 = tf.compat.v1.get_variable('w2', [512,self.n_actions], initializer=w_init, collections=name)
                    b2 = tf.compat.v1.get_variable('b2', [self.n_actions], initializer=b_init, collections=name)
        ...
        self.s = tf.compat.v1.placeholder(tf.float32, [None, 84, 84, 4], name='state')
        self.y = tf.compat.v1.placeholder(tf.float32, [None])
        with tf.compat.v1.variable_scope('eval_net'):
            name = ['eval_net_params', tf.compat.v1.GraphKeys.GLOBAL_VARIABLES]
            self.q_eval = build_layers(self.s, name)

        self.s_ = tf.compat.v1.placeholder(tf.float32, [None, 84, 84, 4], name='s_')
        with tf.compat.v1.variable_scope('tar_net'):
            name = ['tar_net_params', tf.compat.v1.GraphKeys.GLOBAL_VARIABLES]
            self.q_target = build_layers(self.s_, name)

        with tf.name_scope('optimization_step'):
            self.action_mask = tf.compat.v1.placeholder(tf.float32, [None, self.n_actions])
